...
(function () {
    ...
    const OVERLAY_PAGES = {
        'group.html': {
            ...
            isolatedScript: {
                src: 'group.js',
                // v1.6 FIX — CONFIRMED ROOT CAUSE of "hardcoded data until
                // refresh": console showed "Identifier 'db' has already
                // been declared". group.js declares `const db` at its top
                // level; chats.js (loaded natively on chats.html) ALSO
                // declares a top-level `db`. That's a fatal SyntaxError —
                // it kills group.js's ENTIRE script before a single line
                // runs, so registerPage() never fires, initPage() never
                // fires, Firestore is never queried, and the freshly
                // fetched raw HTML (with its hardcoded demo name/count)
                // just sits there untouched. Only a real refresh works,
                // because a refresh loads group.js on its own page with
                // no chats.js around to collide with.
                // The old renameMap only listed 4 names (found one console
                // error at a time: authReadyResolve, authReadyPromise,
                // firebaseConfig, pressTimer) — which is exactly how 'db'
                // slipped through. Fixed properly this time: EVERY
                // top-level let/const group.js declares is renamed, so no
                // future name group.js happens to share with chats.js can
                // cause this again. (Checked: none of these names are
                // referenced directly from group.html's onclick attributes,
                // and none are used as Firestore document field keys —
                // only as local variables — so renaming all of them is
                // safe.)
                renameMap: {
                    firebaseConfig: 'firebaseConfig_group',
                    auth: 'auth_group',
                    db: 'db_group',
                    MEDIA_UPLOAD_ENDPOINT: 'MEDIA_UPLOAD_ENDPOINT_group',
                    FieldValue: 'FieldValue_group',
                    currentUsername: 'currentUsername_group',
                    authReadyResolve: 'authReadyResolve_group',
                    authReadyPromise: 'authReadyPromise_group',
                    groupSlug: 'groupSlug_group',
                    groupData: 'groupData_group',
                    isGroupAdmin: 'isGroupAdmin_group',
                    groupUnsub: 'groupUnsub_group',
                    groupPostsUnsub: 'groupPostsUnsub_group',
                    groupMessagesUnsub: 'groupMessagesUnsub_group',
                    groupFeedLoaded: 'groupFeedLoaded_group',
                    currentInviteTab: 'currentInviteTab_group',
                    inviteFriends: 'inviteFriends_group',
                    inviteSuggested: 'inviteSuggested_group',
                    inviteFollowers: 'inviteFollowers_group',
                    inviteGroups: 'inviteGroups_group',
                    chatLockOn: 'chatLockOn_group',
                    autoTranslateOn: 'autoTranslateOn_group',
                    lowDataOn: 'lowDataOn_group',
                    voiceA11yOn: 'voiceA11yOn_group',
                    feedRendered: 'feedRendered_group',
                    chatMessages: 'chatMessages_group',
                    replyTarget: 'replyTarget_group',
                    pinnedMessageId: 'pinnedMessageId_group',
                    activeTrayMsgId: 'activeTrayMsgId_group',
                    pressTimer: 'pressTimer_group',
                    typingIndicatorActive: 'typingIndicatorActive_group',
                    ajoContributors: 'ajoContributors_group',
                    EMOJI_CATEGORIES: 'EMOJI_CATEGORIES_group',
                    currentEmojiCat: 'currentEmojiCat_group',
                    recTimer: 'recTimer_group',
                    recSeconds: 'recSeconds_group',
                    demoTypingTimeout: 'demoTypingTimeout_group',
                    demoMsgTimeout: 'demoMsgTimeout_group',
                    groupMuted: 'groupMuted_group',
                    CHAT_THEMES: 'CHAT_THEMES_group',
                    currentThemeIndex: 'currentThemeIndex_group',
                    groupMembers: 'groupMembers_group',
                    pendingRequests: 'pendingRequests_group',
                    feedPosts: 'feedPosts_group',
                    composerType: 'composerType_group',
                    composerSelectedBg: 'composerSelectedBg_group',
                    composerTaggedUsers: 'composerTaggedUsers_group',
                    composerLocation: 'composerLocation_group',
                    composerFeeling: 'composerFeeling_group',
                    composerSelectedGroups: 'composerSelectedGroups_group',
                    addGroupsCache: 'addGroupsCache_group',
                    feelingOptions: 'feelingOptions_group',
                    composerBgMap: 'composerBgMap_group',
                    composerUploadedMediaUrl: 'composerUploadedMediaUrl_group',
                    composerUploadedMediaType: 'composerUploadedMediaType_group',
                    groupHighlights: 'groupHighlights_group',
                    currentStoryIndex: 'currentStoryIndex_group'
                }
            },
            styles: [
