...
function _videosPopstateHijack() {
    // Babu preventDefault/stopImmediatePropagation a nan — babu amfaninsu
    // akan popstate. Muna dogaro da window.__npProfileOverlay kadai (an
    // saita shi = true a openVideosOverlay), wanda router.js DA KANSA ke
    // dubawa a farkon popstate handler dinsa domin ya bar navigateTo().
    // BABU pushState/history.back() a wannan sigar — wancan tsarin ya
    // haifar da race/stacking bugs (blank feed, back yana komawa wurin
    // da ba a so, back icon mai makale). Wannan sigar mafi sauki ce:
    // ba mu taba history stack ba kwata-kwata, kawai muna "kama" popstate
    // guda daya lokacin da overlay yake bude, mu rufe ta.
    closeVideosOverlay();
}
...
window.openVideosOverlay = async function() {
    ...
    // fita/lalata shafi. BABU pushState a nan — mun cire tsarin da yake
    // push/back domin shi ne yake haifar da stacking bugs (blank feed,
    // back yana komawa wurin da ba a so, back icon mai makale).
    ...
    // "Shirye" = ana bincika DOM din KANSA (akwai post-card ko empty
    // state a ciki), BA wani flag daban ba — flag na iya fita daga
    // tsari (stuck), DOM din baya taba karya ba, don haka babu "blank
    // har sai an refresh" kuma: idan babu content, KULLUM za a sake
    // kokarin loda ta a duk lokacin da aka bude overlay.
    if (feedEl && feedEl.querySelector('.post-card, .empty-videos')) {
        return; // shirye tun kafin a danna — nan take.
    }
    _startVideosLoad();
};
...
window.closeVideosOverlay = function() {
    ...
    window.__npProfileOverlay = false; // kar ya "kama" wani popstate da bai shafe mu ba nan gaba
...
// SAFETY NET: idan ta yiwu ta kasance overlay ya rage a bude (display
// block) alhali user ya riga ya SPA-navigate zuwa wata page (shop,
// services, da sauransu) — koda ta yaya hakan ya faru — mu tilasta
// rufe ta nan da nan, domin back icon dinta ko black overlay dinta ba
// zai taba "makale" ya bayyana a kan wata page ba.
...
setTimeout(() => {
    ...
    const fEl = document.getElementById('videoFeed');
    if (!fEl || fEl.querySelector('.post-card, .empty-videos')) return;
    _startVideosLoad();
}, 2800);
