/* ============================================================
   VIDEOS.JS — SPA page na yau da kullum (videos.html), kamar
   sauran pages (social, shop, services). Ana kiran initVideosPage()
   ta NexusRouter a duk lokacin da aka SPA-navigate zuwa nan, da
   destroyVideosPage() a duk lokacin da aka bar wannan page.
   Business logic (loadVideos, algorithm scoring, watch tracking)
   bai canja ba — kawai an cire duk abinda ya kasance na musamman
   ga tsarin "overlay a cikin social.html" (back-button hijack,
   popstate hijack, background content-prefetch, da dai sauransu),
   domin router.js shi kadai ke da alhakin canza page da history.
   ============================================================ */
...
// Domin dakatar da "flash" na sabon content bayan mun bar page din:
// idan mutum ya SPA-navigate zuwa wata page YAYIN wani fetch/listener
// yana gudana, wadannan callbacks/listeners ba za su sake taba DOM ba,
// domin za mu duba _videosPageAlive kafin kowace innerHTML/textContent.
let _videosPageAlive = false;
let _videosSnapshotUnsubs = [];
    ...
    try {
        if (reset) {
            ...
            try {
                ...
                if (!_videosPageAlive) { isLoading = false; return; }
        ...
        batch.forEach(v => {
            ...
            const _unsub = db.collection('nexus_contributions')
              .where('postId','==',v.id).where('parentId','==',null)
              .onSnapshot(s => {
                  if (!_videosPageAlive) return;
                  const el=document.getElementById(`comment-count-${v.id}`); if(el) el.textContent=s.size;
              });
            _videosSnapshotUnsubs.push(_unsub);
        });
...
// ============================================================
// PAGE INIT / DESTROY (NexusRouter) — na yau da kullum, kamar
// social.html/shop.html/services.html.
// ============================================================
...
function destroyVideosPage() {
    _videosPageAlive = false;
    ...
    _videosSnapshotUnsubs.forEach(fn => { try { fn(); } catch (e) {} });
    _videosSnapshotUnsubs = [];
}

if (window.NexusRouter) {
    NexusRouter.registerPage('videos.html', { init: initVideosPage, destroy: destroyVideosPage });
}
