...
// ============================================================
// HEADER: boye lokacin ana scroll UP, sake bayyana kai-tsaye bayan
// sakan 2 na tsayawa (koda ta wace hanya scroll ya tsaya). Musamman
// ga social.html kadai — an tsare shi da named functions domin a
// iya cire su daga listeners a destroySocialPage() idan an bar
// social.html, in ba haka ba za su zauna a `window` har abada.
// ============================================================
(function () {
...
let _headerLastScrollY = 0;
let _headerAutoShowTimer = null;
function _socialHeaderScrollHandler() {
    const header = document.querySelector('header');
    if (!header) return;
    const y = window.scrollY;

    if (y <= 60) {
        // Kusa da saman page — koyaushe a nuna header.
        header.style.transform = 'translateY(0)';
    } else if (y < _headerLastScrollY) {
        // Ana scroll UP — boye header.
        header.style.transform = 'translateY(-110%)';
    }
    _headerLastScrollY = y;

    clearTimeout(_headerAutoShowTimer);
    _headerAutoShowTimer = setTimeout(() => {
        header.style.transform = 'translateY(0)';
    }, 2000);
}
function _socialHeaderScrollReset() {
    clearTimeout(_headerAutoShowTimer);
    _headerAutoShowTimer = null;
    _headerLastScrollY = 0;
    const header = document.querySelector('header');
...
function initSocialPage() {
    ...
    _socialHeaderScrollReset();
    window.addEventListener('scroll', _socialHeaderScrollHandler, { passive: true });
}
...
function destroySocialPage() {
    ...
    window.removeEventListener('scroll', _socialHeaderScrollHandler);
    _socialHeaderScrollReset();
}
