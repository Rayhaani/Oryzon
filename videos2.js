/* ============================================================
   VIDEOS.JS — an fitar da wannan daga inline <script> a videos.html
   domin ya yi aiki daidai a karkashin NEXUS SPA ROUTER (router.js).
   Business logic din asali BAI CANJA ba — an kawai:
   1) Mika DOM lookups (feedEl/loadMoreTrigger) da boot-call zuwa
      pageInit() domin su sake gudana kowane lokaci da aka koma
      videos.html ta SPA.
   2) Mika popstate/click/scroll listeners zuwa named functions
      domin a iya cire su a pageDestroy() idan an bar videos.html.
   3) Tsare infinite-scroll observer da insertAdjacentHTML patch
      domin kada su tara sabani a duk sake-shigowa.
   ============================================================ */

// MUHIMMI: an cire firebase.initializeApp()/const db/const storage
// daga nan — nexus-core.js (wanda ke loda a KOWACE page, SAU DAYA
// kacal a duk rayuwar app din) shine YANZU ke da alhakin wannan gaba
// daya. videos.js yana amfani da `db`/`storage`/`analytics` da suka
...
// Ba a saka su const domin #page-content yana sake gina DOM din a
// kowace SPA navigation — dole a sake nemansu a pageInit() kowane
// lokaci, ba kawai sau daya ba lokacin farko da videos.js ta loda.
let feedEl          = null;
let loadMoreTrigger = null;

// ── refs domin destroy() ya iya cire listeners/observers daidai ──
let _infiniteScrollObserver = null;
let _infiniteScrollSentinel = null;
let _scrollT;

// ── 🔴 HIJACKING BACK BUTTON (KAFIN TEMPLATE.JS YA TAƁA SHI) ──
// An mika su zuwa named functions domin a iya cire su (removeEventListener)
// a pageDestroy() — in ba haka ba, sun kasance suna zaman har abada a
// window/document har ma bayan mutum ya bar videos.html zuwa wata page.
...
function _scoreVideo(video) {
    ...
    let score    = 0;
    // 1. Watch completion rate (45%) — TikTok's #1 signal
...
// ── INFINITE SCROLL ──
// GYARAN SPA: an tsare shi domin kada ya tara sentinel/observer sabo
// a duk lokacin da aka koma videos.html — a maimakon haka ya share
// tsohon observer/sentinel kafin ya kirkiri sabo.
function setupInfiniteScroll() {
    ...
    document.body.appendChild(_infiniteScrollSentinel);
...
function startFirstImmersive() {
    ...
    if (!history.state || !history.state.immersive) {
        history.pushState({ immersive: true }, '');
    }
}

// Kira bayan videos sun loda
// GYARAN SPA: an tsare wannan monkey-patch da flag domin kada a sake
// yin patch a saman patch idan an sake shigo videos.html (wanda zai
// haddasa kiran startFirstImmersive() sau da yawa a jere).
...
async function pageInit() {
    // ── AUTH GUARD (ana sake dubawa kowane shigowa) ──
    if (!localStorage.getItem('nexus_user_session')) { location.href = 'login.html'; return; }

    // Sabon shigowa zuwa videos.html = sabon DOM a #page-content,
    // don haka dole a sake nemo wadannan elements sabo.
    feedEl          = document.getElementById('videoFeed');
    loadMoreTrigger = document.getElementById('loadMoreTrigger');

    // Sabunta state domin sabon shiga (kar tsohon filter/page ya rage).
    activeFilter = 'all';
    lastVisible  = null;
    isLoading    = false;
    allLoaded    = false;
    ...
    window.addEventListener('scroll', _videosScrollPriorityHandler, { passive: true });

    const savedPic = localStorage.getItem('userProfilePic');
    const img = document.getElementById('footerProfileImg');
    const u = localStorage.getItem('nexus_user_session') || 'user';
    if (savedPic && img) img.src = savedPic;
    else if (img) img.src = `https://api.dicebear.com/7.x/bottts/svg?seed=${u}`;
...
function pageDestroy() {
    ...
    window.removeEventListener('scroll', _videosScrollPriorityHandler);
    clearTimeout(_scrollT);
...
if (window.NexusRouter) {
    NexusRouter.registerPage('videos.html', { init: pageInit, destroy: pageDestroy });
}
pageInit();
