...
function rate(n) {
    ...
    stars.forEach((s, index) => { s.style.color = index < n ? '#FFB800' : 'rgba(245,245,220,0.15)'; });
...
async function submitReview() {
    ...
    if (!text || selectedStars === 0 || !activeProductId) return;
    ...
    try {
        ...
        const reviewsCol = _pp_collection(_pp_db, 'products', activeProductId, 'reviews');
        await _pp_addDoc(reviewsCol, {
            rating: selectedStars,
            text: text,
            userName: currentUser.name,
            createdAt: _pp_serverTimestamp()
        });

        textEl.value = '';
        selectedStars = 0;
        document.querySelectorAll('#star-box span').forEach(s => s.style.color = 'rgba(245,245,220,0.15)');
    ...
    btn.style.color = 'rgba(245,245,220,0.35)';
...
async function loadProductReviews(productId) {
    ...
    try {
        await ensureProductsPageFirebase();
        const reviewsCol = _pp_collection(_pp_db, 'products', productId, 'reviews');
        const q = _pp_query(reviewsCol, _pp_orderBy('createdAt', 'desc'));
        const snap = await _pp_getDocs(q);
        const reviews = [];
        snap.forEach(d => reviews.push(d.data()));
        renderReviewsSection(reviews);
    } catch (err) {
        console.error('loadProductReviews failed:', err);
    }
}
...
function renderReviewsSection(reviews) {
    ...
    if (summaryEl) {
        summaryEl.innerHTML = count > 0
            ? `<span style="font-size:20px;font-weight:800;color:#F5F5DC;">${avg.toFixed(1)}</span>
               <span style="color:#FFB800;font-size:14px;letter-spacing:1px;">${"★".repeat(roundedAvg)}${"☆".repeat(5 - roundedAvg)}</span>
               <span style="font-size:13px;color:rgba(245,245,220,0.5);">(${count})</span>`
            : `<span style="font-size:13px;color:rgba(245,245,220,0.5);">No reviews yet — be the first!</span>`;
    }

    const badgeEl = document.getElementById('product-rating-badge');
    if (badgeEl) {
        badgeEl.innerHTML = count > 0
            ? `${avg.toFixed(1)}
               <div class="flex text-[8px] gap-0.5">
                   ${'<i class="fa-solid fa-star"></i>'.repeat(roundedAvg)}${'<i class="fa-regular fa-star"></i>'.repeat(5 - roundedAvg)}
               </div>`
            : `New`;
    }
    ...
    feed.innerHTML = reviews.map(r => {
        ...
        const stars = Math.max(0, Math.min(5, r.rating || 0));
        return `
        <div style="border-top:1px solid rgba(255,255,255,0.08);padding:15px 0;font-family:'Quicksand',sans-serif;">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">
                <div style="width:32px;height:32px;border-radius:50%;background:rgba(255,255,255,0.1);display:flex;align-items:center;justify-content:center;font-weight:800;font-size:12px;color:#F5F5DC;flex-shrink:0;">${name[0].toUpperCase()}</div>
                <div style="flex-grow:1;">
                    <div style="display:flex;justify-content:space-between;align-items:center;">
                        <h4 style="font-size:12px;font-weight:800;margin:0;color:#F5F5DC;">${name}</h4>
                        <span style="font-size:10px;color:rgba(245,245,220,0.4);">${dateStr}</span>
                    </div>
                    <div style="color:#FFB800;font-size:10px;letter-spacing:1px;">${"★".repeat(stars)}${"☆".repeat(5 - stars)}</div>
                </div>
            </div>
            <p style="font-size:12px;color:rgba(245,245,220,0.75);line-height:1.4;margin:0 0 0 42px;">${r.text || ''}</p>
        </div>`;
    }).join('');
}
...
let _pp_collection, _pp_addDoc, _pp_getDocs, _pp_query, _pp_orderBy, _pp_serverTimestamp, _pp_limit, _pp_where;
async function ensureProductsPageFirebase() {
    if (_pp_db) return;
    const { initializeApp, getApps, getApp } = await import("https://www.gstatic.com/firebasejs/11.0.0/firebase-app.js");
    const { getFirestore, doc, getDoc, collection, addDoc, getDocs, query, orderBy, serverTimestamp, limit, where } = await import("https://www.gstatic.com/firebasejs/11.0.0/firebase-firestore.js");
    ...
    _pp_doc = doc;
    _pp_getDoc = getDoc;
    _pp_collection = collection;
    _pp_addDoc = addDoc;
    _pp_getDocs = getDocs;
    _pp_query = query;
    _pp_orderBy = orderBy;
    _pp_serverTimestamp = serverTimestamp;
    _pp_limit = limit;
    _pp_where = where;
}
...
async function loadSellerInfo(vendorId) {
    try {
        ...
        const salesEl = document.getElementById('seller-stat-sales');
        if (salesEl) salesEl.textContent = (v.sold || 0).toLocaleString();

        const itemsEl = document.getElementById('seller-stat-items');
        if (itemsEl) {
            const productsCol = _pp_collection(_pp_db, "products");
            const [byVendorId, byVendor, bySellerId] = await Promise.all([
                _pp_getDocs(_pp_query(productsCol, _pp_where("vendorId", "==", vendorId))),
                _pp_getDocs(_pp_query(productsCol, _pp_where("vendor", "==", vendorId))),
                _pp_getDocs(_pp_query(productsCol, _pp_where("sellerId", "==", vendorId)))
            ]);
            const uniqueIds = new Set();
            [byVendorId, byVendor, bySellerId].forEach(snap => snap.forEach(d => uniqueIds.add(d.id)));
            itemsEl.textContent = uniqueIds.size;
        }

        const ratingEl = document.getElementById('seller-stat-rating');
        if (ratingEl) {
            const reviewsSnap = await _pp_getDocs(_pp_collection(_pp_db, "vendors", vendorId, "reviews"));
            if (reviewsSnap.empty) {
                ratingEl.textContent = 'New';
            } else {
                let sum = 0;
                reviewsSnap.forEach(d => sum += Number(d.data().rating) || 0);
                ratingEl.textContent = (sum / reviewsSnap.size).toFixed(1);
            }
        }
    } catch (err) {
...
async function loadRecommendedProducts(excludeId) {
    const grid = document.getElementById('premium-vendors-grid');
    if (!grid) return;
    try {
        await ensureProductsPageFirebase();
        const q = _pp_query(_pp_collection(_pp_db, 'products'), _pp_limit(8));
        const snap = await _pp_getDocs(q);
        const items = [];
        snap.forEach(d => {
            if (d.id === excludeId) return;
            const p = d.data();
            items.push({ id: d.id, name: p.name || '', price: p.price || '', image: p.image || p.img || '', location: p.location || '' });
        });
        const picks = items.slice(0, 6);
        grid.innerHTML = picks.length
            ? picks.map(productCardHTML).join('')
            : `<div style="grid-column:1/-1;text-align:center;color:rgba(245,245,220,0.4);padding:30px 0;font-size:12px;">No other products yet.</div>`;
    } catch (err) {
        console.error('loadRecommendedProducts failed:', err);
        grid.innerHTML = `<div style="grid-column:1/-1;text-align:center;color:rgba(245,245,220,0.4);padding:30px 0;font-size:12px;">Could not load recommendations.</div>`;
    }
}
...
applyProductData(nameVal, priceVal, imgUrl, descVal, galleryVal, soldVal);
...
function applyProductData(name, price, img, desc, extraImages, soldCount) {
    ...
    const soldEl = document.getElementById('product-sold-badge');
    if (soldEl) soldEl.textContent = (soldCount || 0).toLocaleString();
    ...
    function restoreAvatarPosition() {
        ...
        if (saved) {
            try {
                ...
                // Wata dadaddiyar adana matsayi da take kusa da back button (sama-hagu) —
                // a maye gurbinta da sabon default din maimakon a bar overlap din ya ci gaba.
                if (!(x < 50 && y < 50)) {
                    el.style.left = x + 'px'; el.style.top = y + 'px'; return;
                }
            } catch(e) {}
        }
        el.style.left = '10px';
...
${vendorInfo}${learningExamples}`;
    ...
    // Ire-iren wadannan su kadai ake bukata a fili (window.*) — ko dai domin
    // openVendorChatOverlay()/closeVendorChatOverlay() (a wajen wannan IIFE)
    // suna kiransu, ko kuma domin onclick="..." a cikin HTML na vendor-chat
    // yana kiransu. Komai kuma da ke cikin wannan IIFE ya rage a boye,
    // domin kada sunayen gama-gari (db, params, auth, vendorId, da sauransu)
    // su yi karo da wani script na wata page idan aka riga aka loda ta a
    // wannan SPA session guda.
    window.bootVendorChat = bootVendorChat;
    window.destroyVendorChat = destroyVendorChat;
    window.cancelContextBar = cancelContextBar;
    window.cancelMediaCompose = cancelMediaCompose;
    ...
    window.rotateActiveMedia = rotateActiveMedia;
    window.sendMediaCompose = sendMediaCompose;
    window.startScrub = startScrub;
    window.toggleBot = toggleBot;
    window.toggleInputUI = toggleInputUI;
