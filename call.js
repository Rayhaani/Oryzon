...
(function bootNexusCall() {
    // BUGFIX: DOMContentLoaded already fired long before this script is
    // inserted during SPA navigation (router.js appends it AFTER the
    // document has fully loaded), so that listener never runs in the SPA
    // flow. firebase-*-compat.js are guaranteed loaded before this file
    // (see PAGE_SCRIPTS['chat-interior.html'] order in router.js), so it
    // is safe to run immediately on both native load and SPA navigation.
