/* ============================================================
   ...
   ============================================================ */
    ...
    if (window.NexusRouter && typeof window.NexusRouter.navigateTo === 'function') {
        ...
        var authReadyPromise = new Promise((resolve) => { authReadyResolve = resolve; });
...
if (window.NexusRouter && typeof window.NexusRouter.navigateTo === 'function') {
    ...
    function chatsSwitchTabImpl(element, tabId) {
    ...
    if (tabId !== 'friends') {
        ...
        runOnChatsInit(() => {
            ...
            if (savedTab && savedTab !== 'personal') {
                ...
                if (btn) chatsSwitchTabImpl(btn, savedTab);
            }
...
function handleStatusUpload(input, fileType) {
    ...
    if (!myId) {
        chatsShowToastImpl('Connection not ready yet, please wait a moment.');
        return;
    }
    ...
    suggestPrivacyTier(file).then(suggested => {
        if (suggested && suggested !== uploadPrivacy) {
            chatsShowToastImpl(`💡 AI suggests: ${suggested === 'closeFriends' ? 'Close Friends' : 'Friends'} for this photo.`);
        }
    });
}
...
function confirmPostStatus() {
    ...
    fetch(`${BACKEND_URL}/upload`, { method: 'POST', body: formData })
        ...
        .then(() => {
            chatsShowToastImpl('Status posted! 🚀');
            cancelComposePreview();
        })
        .catch((error) => {
            console.error('Error uploading:', error);
            chatsShowToastImpl('Something went wrong while posting your status.');
        })
...
function postTimeCapsule(previewImg, unlockInHours, caption) {
    ...
    db.collection('statusData').add({
        ...
        visibility: uploadPrivacy,
        viewerIds: []
    }).then(() => chatsShowToastImpl('Time-Capsule scheduled! ⏳'));
}
...
function chatsShowToastImpl(msg) {
...
function toggleMuteUser(userId) {
    ...
    ).then(() => { chatsShowToastImpl(muted ? 'Unmuted.' : 'Muted.'); rebuildStoryGroups(); });
}

function deleteMyStatus() {
    const myCard = statusData.find(s => s.isMe);
    if (!myCard || !myCard.slides.length) { chatsShowToastImpl('No status to delete.'); return; }
    const batch = db.batch();
    myCard.slides.forEach(s => batch.delete(db.collection('statusData').doc(s.id)));
    batch.commit().then(() => chatsShowToastImpl('Status deleted.'));
}
...
function forwardSlide(slideId) {
    ...
    }).then(() => chatsShowToastImpl(`Forwarded to ${target}.`))
      .catch(() => chatsShowToastImpl('Could not forward. Please try again.'));
}
...
function shareSlide(slideId) {
    ...
    if (navigator.share) {
        ...
        navigator.clipboard.writeText(slide.previewImg)
            .then(() => chatsShowToastImpl('Link copied — share it anywhere.'))
            .catch(() => chatsShowToastImpl('Could not copy link.'));
    }
}
...
function editSlideCaption(slideId) {
    ...
    db.collection('statusData').doc(slideId).update({ caption: newCaption })
        .then(() => { chatsShowToastImpl('Caption updated.'); slide.caption = newCaption; })
        .catch(() => chatsShowToastImpl('Could not update caption.'));
}
...
function deleteSpecificSlide(slideId) {
    ...
    db.collection('statusData').doc(slideId).delete()
        .then(() => { chatsShowToastImpl('Status deleted.'); renderMyStatusList(); })
        .catch(() => chatsShowToastImpl('Could not delete. Please try again.'));
}
...
function renderSlideContent(slide, profile) {
    ...
    if (isCapsuleLocked(slide)) {
        ...
        canvas.appendChild(div);
        chatsStartCapsuleCountdownImpl(slide.unlockAt);
        return;
    }
...
function chatsStartCapsuleCountdownImpl(unlockAt) {
...
function initChatsPage() {
    // MUHIMMI: pages.js (da sauran page scripts) suna da NASU
    // switchTab/showToast/startCapsuleCountdown a GLOBAL scope (babu
    // IIFE, bisa gangan, domin onclick="..." su same su). Duk wanda ya
    // loda/gudana a KARSHE (a wannan session) shi ke "mamaye" sunan
    // global din. Don haka, a KOWACE shiga chats.html, dole mu SAKE
    // dora wadannan sunaye zuwa NASU (chats.js) version, domin
    // onclick="switchTab(...)"/"showToast(...)" a wannan page su kira
    // daidai function din da ya dace da chats.html.
    window.switchTab = chatsSwitchTabImpl;
    window.showToast = chatsShowToastImpl;
    window.startCapsuleCountdown = chatsStartCapsuleCountdownImpl;
