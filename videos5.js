/* ============================================================
   ...
   3) An kara background PREFETCH (duba karshen file) domin overlay
      ya bude NAN TAKE (kamar Instagram) — babu jiran fetch a
      lokacin danna icon din. closeVideosOverlay() ba ya sake
      share/wanke feedEl ba, domin sake bude ya ci gaba ya zama
      nan take shi ma.
   ============================================================ */
...
let _videosPrefetched = false;

let _infiniteScrollObserver = null;
let _infiniteScrollSentinel = null;
let _scrollT;

function _videosPopstateHijack() {
    // BABU preventDefault/stopImmediatePropagation a nan — ba su da
    // wani amfani akan popstate na window (an riga an canza URL/history
    // kafin wannan event ya harba, kuma router.js's listener (wanda aka
    // yi register tun farkon page) YANA GUDANA KAFIN wannan namu, ko da
    // capture=true, domin dukkansu suna kan window (AT_TARGET phase —
    // registration order ne ke tsara su, ba capture flag ba). Shi ya sa
    // a da router.js yake sake maido/wanke #page-content kafin mu rufe
    // overlay, wanda ke haifar da hargitsewar shafi lokacin danna back.
    // Maimakon haka, muna amfani da window.__npProfileOverlay — flag
    // din da router.js DA KANSA ke dubawa a farkon popstate handler
    // dinsa, ya bar navigateTo() idan flag=true (an riga an tanadar
    // wannan tsari a router.js domin irin wannan yanayi na overlays).
    closeVideosOverlay(true);
}
    ...
    try {
        if (reset) {
            ...
            if (raw.length === 0) {
                ...
                feedEl.classList.add('ready');
                loadMoreTrigger.style.display = 'none';
                _videosPrefetched = true; // ainihin gaskiyar sakamako ne — babu bidiyo, ba kuskure ba
                isLoading = false; return;
            }
            _videosPrefetched = true; // an samu content na gaskiya — babu bukatar sake kokari
            const perAuthor = {};
            _scoredCache = raw
        ...
        if (reset) {
            ...
            if (!_infiniteScrollObserver) setupInfiniteScroll();
        }
...
window.openVideosOverlay = async function() {
    ...
    // Domin hardware/browser BACK button ya rufe overlay maimakon ya
    // fita/lalata shafi — duba bayanin _videosPopstateHijack a sama.
    window.__npProfileOverlay = true;
    window.history.pushState({ nexusVideosOverlay: true }, '', window.location.href);
    window.addEventListener('popstate', _videosPopstateHijack);
    ...
    if (!_watchObserver) _setupWatchTracking();
    if (!_infiniteScrollObserver) setupInfiniteScroll();

    if (_videosPrefetched) {
        // An riga an shirya (prefetch) tun kafin a danna — nuna NAN
        // TAKE, kamar Instagram. Babu sake fetch/reset.
        return;
    }

    // Ko dai an danna kafin background prefetch ta gama, KO kuma
    // yunkurin farko ya kasa (network/offline) — a KOWANNE hali, wannan
    // shine kadai wurin da ke kokarin loda content, don haka babu
    // "empty forever" — kowace bude overlay tana sake gwadawa har sai
    // ta samu nasara.
    _startVideosLoad();
};

window.closeVideosOverlay = function(fromPopstate) {
    const overlay = document.getElementById('videosOverlay');
    ...
    window.removeEventListener('popstate', _videosPopstateHijack);
    ...
    // BABU feedEl.innerHTML='' a nan — muna barin content dinsa a
    // shirye a DOM domin sake bude overlay din nan gaba ya kasance
    // NAN TAKE ma (ba tare da sake fetch/render ba).

    // Idan an rufe ta hanyar UI (ba popstate ba tukuna), mu cire
    // history entry din da muka push a openVideosOverlay(), domin
    // BACK na gaba (bayan an rufe ta hanyar tap) ya ci gaba zuwa
    // inda ake KAFIN mu bude overlay, ba wani "phantom" state ba.
    if (!fromPopstate && window.history.state && window.history.state.nexusVideosOverlay) {
        window.__npProfileOverlay = true;
        window.history.back();
    }
};

// Yunkurin loda videos na farko (asirce ko a lokacin danna icon din).
// Ana kiran shi DAGA prefetch timeout KO daga openVideosOverlay — a
// KOWANNE hali, `_videosPrefetched` ba ya zama TRUE sai bayan an SAMU
// content na gaskiya (ko kuma an tabbata BABU wani video, wanda shi ma
// sakamako ne na gaskiya) — idan fetch ya kasa (offline/network), an
// bar shi FALSE domin gwaji na gaba (bude na gaba) ya sake kokari.
let _videosLoadInFlight = false;
function _startVideosLoad() {
    if (_videosLoadInFlight) return;
    feedEl          = feedEl          || document.getElementById('videoFeed');
    loadMoreTrigger = loadMoreTrigger || document.getElementById('loadMoreTrigger');
    if (!feedEl || !loadMoreTrigger) return;
    _videosLoadInFlight = true;
    ...
    _loadUserContext()
        ...
        .finally(() => { _videosLoadInFlight = false; });
}

/* ============================================================
   PREFETCH — jira dan kadan bayan page ta gama load, sannan mu
   fara loda bidiyo a ASIRCE (overlay din yana display:none tukuna,
   babu UI da user ke gani), domin da zaran an danna icon din
   videos, content ya riga ya kasance a shirye — kamar Instagram
   Reels. Layin sync da nexus-feed.js amma an ɗan jinkirta shi
   (2800ms maimakon 2500ms) domin kada dukkan bidiyo/hoto na
   nexus-feed da videos su nemi network daidai lokaci guda.
   ============================================================ */
setTimeout(() => {
    if (_videosPrefetched) return;
    if (!localStorage.getItem('nexus_user_session')) return;
