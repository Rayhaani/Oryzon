...
.follow-text-link:hover {
    ...
    box-shadow: 0 0 30px rgba(253,224,141,0.6);
    letter-spacing: 4px;
}
...
.gift-emoji {
    font-size: 14px;
    filter: drop-shadow(0 0 4px rgba(253,224,141,0.5));
    animation: giftWobble 4s infinite;
}

.gift-btn-nexus span:last-child {
    font-size: 10px;
    color: #ffffff;
    font-weight: 600;
...
/* ===== POST HEADER CURVED BOTTOM ===== */
.post-header {
    border-bottom-left-radius: 40px 15px !important;
    border-bottom-right-radius: 40px 15px !important;
    border-bottom: 1px solid rgba(253,224,141,0.2) !important;
    background: linear-gradient(180deg, rgba(253,224,141,0.12) 0%, transparent 100%) !important;
    backdrop-filter: blur(20px) !important;
    -webkit-backdrop-filter: blur(20px) !important;
}

/* ===== INTERACTION BAR ICONS — FARI ===== */
.post-capsule i, .capsule i {
    color: #ffffff !important;
    font-size: 15px !important;
}
...
.nexus-badge {
    ...
    clip-path: polygon(
        ...
        100% 28%, 95% 39%, 100% 50%, 95% 61%, 100% 72%,
        91% 80%, 91% 91%, 80% 91%, 72% 100%, 61% 95%,
        50% 100%, 39% 95%, 28% 100%, 20% 91%, 9% 91%,
        9% 80%, 0% 72%, 5% 61%, 0% 50%, 5% 39%, 0% 28%,
        9% 20%, 9% 9%, 20% 9%, 28% 0%, 39% 5%
    );
    margin-left: 5px;
