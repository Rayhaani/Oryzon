...
.feed-container {
    ...
    overflow: hidden !important;
        ...
        /* ============================================================
           GYARA/SABUNTA: an sake tsara duk transition tsakanin carousel
           da grid a matsayin CROSSFADE + SCALE-MORPH mai tsantsar CSS
           (ba a dogara da JS "animation restart hack" ba kwata-kwata
           yanzu). Wannan yana kawar da matsalar "duhu ya bayyana" (flash)
           domin track da grid koyaushe suna zama a saman juna (position:
           absolute, matakin girman carousel guda daya), suna canza
           opacity/transform TARE a lokaci daya — babu wani lokaci da
           komai ba ya nan. background a carousel yana tabbatar da
           babu wani "hole" da zai bayyana ko yaushe. ============ */
        .post-media-carousel {
            position: relative !important;
            width: 100% !important;
            overflow: hidden !important;
            background: #0a0a0a !important;
            transition: height 0.45s cubic-bezier(0.65,0,0.35,1) !important;
        }
        ...
        .post-media-track::-webkit-scrollbar { display: none !important; }
        ...
        .post-media-slide .post-media { width: 100% !important; }

        /* Dots — same small-bullet visual language as the post-header
           menu dots, adapted to sit visibly on top of photos/videos */
        .post-media-dots {
            position: absolute !important;
        ...
        .post-media-dots .post-media-dot {
            width: 5px !important;
        ...
        .post-media-dots .post-media-dot.active {
            background: var(--premium-gold) !important;
            width: 6px !important;
        ...
        /* Counter badge — top-right corner, e.g. "1/4" */
        .post-media-counter {
            position: absolute !important;
            top: 10px !important;
            right: 10px !important;
        ...
        /* Grid view — koyaushe a cikin flow (position:absolute, layer
           daya da track), ana sarrafa bayyanarsa ta hanyar opacity/
           transform/filter KAWAI (ba display:none ba) domin transition
           din ya zama santsi, mai tabbaci, kuma cikakke a koyaushe. */
        .post-media-grid {
            position: absolute !important;
            inset: 0 !important;
        ...
        /* Duk lokacin da carousel take grid-mode: grid ya bayyana a
           sarari (a saman track), track ya boye a bayansa — TARE, a
           lokaci daya, ta hanyar crossfade + subtle blur-focus-pull
           (kamar yadda iOS/premium apps ke yi). */
        .post-media-carousel.grid-mode .post-media-track {
            position: absolute !important;
            top: 0 !important; left: 0 !important;
            width: 100% !important;
            opacity: 0 !important;
        ...
        .post-media-carousel.grid-mode .post-media-grid {
            opacity: 1 !important;
            pointer-events: auto !important;
            transform: scale(1) !important;
            filter: blur(0) !important;
        }
        .post-media-carousel.grid-mode .post-media-grid img,
        .post-media-carousel.grid-mode .post-media-grid video {
            transform: scale(1) !important;
        }
        .post-media-carousel.grid-mode .post-media-dots,
        .post-media-carousel.grid-mode .post-media-counter {
            opacity: 0 !important;
            pointer-events: none !important;
            transform: scale(0.9) !important;
        }

        /* GYARA: A cikin immersive-mode, .post-media (hoton/video na
           carousel) yana da position:absolute, wanda ke fitar da shi
           daga document flow — wannan ke sa .post-media-carousel ya
           rasa tsayi na gaskiya (ya koma 0), sabo da haka JS din
           postCard_toggleGridView yana kulle wannan 0px tsayin kafin
           ya koma grid-mode, wanda ke sa .post-media-grid (mai bukatar
           height:100% daga carousel) ma ya zama 0px = blank. Wannan
...
// ============================================================
// 4e. SHARED HELPER — Multi-media carousel + grid view (NEW)
//     Normalizes whichever field a post uses to store more than
//     one image/video, drives the dots + top-right counter as the
//     user swipes, and toggles the "all-at-once" grid layout.
// ============================================================

// Accepts post.mediaUrls / post.media / post.images (array of strings
// or {url,type} objects) — falls back to the existing single post.mediaUrl
// so posts with only one image/video are completely unaffected.
window.postCard_normalizeMediaItems = function(post) {
    let raw = [];
    if (Array.isArray(post.mediaUrls) && post.mediaUrls.length) raw = post.mediaUrls;
    else if (Array.isArray(post.media) && post.media.length) raw = post.media;
    else if (Array.isArray(post.images) && post.images.length) raw = post.images;
    else if (post.mediaUrl) raw = [{ url: post.mediaUrl, type: post.mediaType || 'image' }];
    ...
    return raw.map(item => {
        ...
        return { url: item.url || item.mediaUrl || '', type: item.type || item.mediaType || 'image' };
    }).filter(m => m.url);
};

// Fires on horizontal scroll of the track — computes which slide is
// centered and updates the matching dot + the "x/y" counter text.
window.postCard_handleCarouselScroll = function(event, postId) {
    const track = event.currentTarget;
    const width = track.offsetWidth || 1;
    const index = Math.max(0, Math.round(track.scrollLeft / width));

    const carousel = track.closest('.post-media-carousel');
    if (!carousel) return;

    const dots = carousel.querySelectorAll('.post-media-dot');
    dots.forEach((dot, i) => dot.classList.toggle('active', i === index));

    const counterEl = carousel.querySelector('.post-media-counter');
    if (counterEl && dots.length) counterEl.textContent = `${index + 1}/${dots.length}`;
...
window.postCard_toggleGridView = function(event, postId) {
    ...
    const grid = card.querySelector('.post-media-grid');
    const track = card.querySelector('.post-media-track');
...
const icon = event.currentTarget.querySelector('i');
    ...
    // 2) Flip the state.
    carousel.classList.toggle('grid-mode', goingToGrid);
    if (icon) icon.className = goingToGrid ? 'fa-solid fa-images' : 'fa-solid fa-table-cells';

    if (goingToGrid) {
        // Sanya stagger delay ga kowanne item KAFIN mu juya grid-mode —
        // domin lokacin da CSS transition ya fara (sanadin canjin class),
        // kowanne item ya rigaya ya san lokacin dalilinsa (i*45ms). Pure
        // CSS transition (ba animation ba) — babu bukatar "restart hack",
        // domin transition yana kunna kansa daidai koyaushe idan darajar
        // property ta canza sakamakon class toggle.
        const gridItems = grid.querySelectorAll('img, video');
...
const rawPic = post.userProfilePic || "https://api.dicebear.com/7.x/bottts/svg?seed=" + (post.username || 'user');
    ...
    // --- Media ---
    // NEW: normalize into a list first — if the post has more than one
    // image/video we render the carousel; otherwise we fall through to
    // the exact original single-media markup (zero change for those posts).
    const nxMediaItems = window.postCard_normalizeMediaItems(post);
    let mediaWrapperHTML = '';

    if (nxMediaItems.length > 1) {
        // ---- NEW: multi-media carousel + dots + counter + grid ----
        const boostTagHTML = post.boosted
            ? `<div class="post-boost-tag"><i class="fa-solid fa-rocket"></i> Sponsored</div>`
            : '';

        const nxFastUrl = (url) => (url && url.includes('cloudinary.com'))
            ? url.replace('/upload/', '/upload/f_auto,q_auto,w_700/')
            : url;

        const slidesHTML = nxMediaItems.map(item => {
            ...
            return `<div class="post-media-slide"><img src="${fUrl}" class="post-media" loading="lazy" alt="post image"></div>`;
        }).join('');
        ...
        mediaWrapperHTML = `
            <div style="position:relative;">
                ...
                <div class="post-media-carousel">
                    <div class="post-media-track" onscroll="postCard_handleCarouselScroll(event,'${postId}')">
                        ${slidesHTML}
                    </div>
                    <div class="post-media-dots">${dotsHTML}</div>
                    <div class="post-media-counter">1/${nxMediaItems.length}</div>
                    <div class="post-media-grid">${gridHTML}</div>
                </div>
            </div>`;
    } else if (post.mediaUrl) {
        // ---- EXISTING single-media path — UNCHANGED ----
   ...
   // --- Pinned tag (NEW) ---
    ...
    // --- Grid-toggle button (NEW) — lives inside the SAME save pill as
    // the bookmark icon, only rendered for multi-media posts, so the
    // right-hand pill holds two icons together the same way the left-hand
    // pill already holds four (like / comment / repost / share). ---
    const gridToggleHTML = nxMediaItems.length > 1
        ? `<div class="post-capsule" onclick="postCard_toggleGridView(event, '${postId}')">
...
<div class="post-username-row" style="display: flex !important; flex-direction: column !important; justify-content: center !important; flex: 1 !important; min-width: 0 !important; margin: 0 0 0 2px !important;">
    ...
    <div style="display:flex; align-items:center; gap:4px; line-height:1.2; min-width:0; margin-top:3px;">
        <span class="post-fullname" style="font-family: inherit; font-size:14px !important; font-weight:900; color:#fff !important; letter-spacing:0.3px; text-shadow: 0 1px 3px rgba(0,0,0,0.6), 0 0 1px rgba(0,0,0,0.45), 0 0 9px rgba(255,255,255,0.35); position:relative; top:1px; flex:0 1 auto; min-width:0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${post.fullName || post.username || 'unknown'}</span>        
        <svg width="16" height="16" viewBox="0 0 24 24" style="flex-shrink: 0; filter: drop-shadow(0 1px 2px rgba(0,0,0,0.5));">
  ...
  <path d="M10.54 16.25L6.85 12.5l1.44-1.44 2.25 2.25 5.27-5.27 1.44 1.44-6.71 6.77z" fill="#000"/>
</svg>
    </div>
    ...
    <div class="post-meta-row" style="display:flex !important; align-items:center !important; gap:4px !important; min-width:0 !important; margin-top:4px !important;">
         <span class="post-username" style="font-family: inherit; font-size:12.5px !important; font-weight:700; color:rgba(255,255,255,0.94) !important; text-shadow: 0 1px 2px rgba(0,0,0,0.5); flex-shrink:0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:55%;"><span style="font-size:0.8em; position:relative; top:-1px;">@</span>${post.username || 'unknown'}</span>
        ${timeStr ? (() => {
            ...
            <span class="post-time"
                  ...
                  data-showing="time"
                  style="font-family: inherit; font-size:11px !important; font-weight: 500 !important; color:rgba(255,255,255,0.92) !important; text-shadow: 0 1px 2px rgba(0,0,0,0.5); flex-shrink:1; min-width:0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; text-transform: ${transformStyle} !important;">${cleanTime}</span>`;
        })() : ''}
    </div>
</div>
