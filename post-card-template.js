...
.feed-container {
    ...
    overflow: hidden !important;
        ...
        /* ============================================================
           ...
           (additive only — does not touch any rule above)
           ============================================================ */
        .post-media-carousel {
            position: relative !important;
            width: 100% !important;
    ...
    animation: nxGridItemIn 0.4s cubic-bezier(0.34,1.56,0.64,1) forwards;
        ...
        /* While in grid mode, the swipe track/dots/counter fade out and
           lift out of the flow (rather than a hard display:none) so the
           height change and the crossfade read as one smooth transition */
...
const icon = event.currentTarget.querySelector('i');
    ...
    if (goingToGrid) {
        ...
        // Stagger each cell's pop-in for a livelier reveal.
        // GYARA: bare "el.style.animation='none'; void el.offsetWidth;
        // el.style.animation='';" trick din baya tabbatar da restart a
        // koda yaushe (musamman idan `.post-media-grid` ta canza daga
        // display:none zuwa display:grid a JUST-in wannan synchronous
        // tick din) — sakamakon shine hotunan sun makale a opacity:0/
        // scale(0.85) na farko har abada (blank). Mun tsananta gyaran
        // ta hanyar amfani da requestAnimationFrame domin mu tabbata
        // browser ya "commit" style din 'none' a wani frame na gaskiya
        // KAFIN a mayar da animation din, wanda shine hanya mafi tabbaci
        // ta sake-kunna CSS animation.
        const gridItems = grid.querySelectorAll('img, video');
        gridItems.forEach(el => { el.style.animation = 'none'; });
        void grid.offsetWidth; // reflow guda daya domin duk items
        ...
        // SAFETY-NET: idan CSS animation din bai kunna ba don wani
        // dalili (misali wani dalili na browser da ba a zata ba), mu
        // tilasta hotunan su bayyana da kansu bayan lokacin da ya kamata
        // animation din ta gama — domin kada grid ta kasance blank har
        // abada koda yaushe.
        setTimeout(() => {
