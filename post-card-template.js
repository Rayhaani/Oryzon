...
.post-card .capsule.liked i {
    color: #ff4d6d !important;
        ...
        /* ============================================================
           ...
           ============================================================ */
        .post-media-carousel { position: relative !important; width: 100% !important; overflow: hidden !important; }
        ...
        /* Grid-toggle capsule — sits to the right of the save capsule
           inside the interaction bar, only rendered for multi-media posts */
        .post-grid-toggle-capsule { margin-left: 4px !important; }
        .post-grid-toggle-capsule .post-capsule { min-width: auto !important; }
    `;
...
window.postCard_toggleGridView = function(event, postId) {
    ...
    const isGrid = carousel.classList.toggle('grid-mode');
    grid.classList.toggle('active', isGrid);

    const icon = event.currentTarget.querySelector('i');
    if (icon) icon.className = isGrid ? 'fa-solid fa-images' : 'fa-solid fa-table-cells';
...
const rawPic = post.userProfilePic || "https://api.dicebear.com/7.x/bottts/svg?seed=" + (post.username || 'user');
   ...
   // --- Pinned tag (NEW) ---
    ...
    // --- Grid-toggle capsule (NEW) — only appears for multi-media posts ---
    const gridToggleHTML = nxMediaItems.length > 1
        ? `<div class="post-action-capsules post-save-capsule post-grid-toggle-capsule">
