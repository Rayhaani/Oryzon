/* ============================================================
   ...
   ============================================================ */
        ...
        var pageUnsub = null;
        var followUnsub = null;
        var postsUnsub = null;
        var viewTracked = false; // don't double-count the same session on every onSnapshot re-fire
        ...
        if (typeof window.openNeuralMenu !== 'function') {
            window.openNeuralMenu = function () { pagesShowToastImpl('Post options'); };
        }
        ...
        async function submitCreatePost() {
            ...
            if (timing !== 'now' && !dateVal) { pagesShowToastImpl('Choose a date/time first'); return; }
            const btn = document.getElementById('createPostSubmitBtn');
            btn.textContent = 'Publishing...';
            try {
                const authUser = await authReadyPromise;
                if (!authUser) { pagesShowToastImpl('You need to be signed in.'); btn.textContent = 'Publish'; return; }
                const postDoc = {
                    pageId: pageSlug,
                ...
                pagesShowToastImpl(timing === 'now' ? 'Post published' : timing === 'capsule' ? '🔒 Time-capsule post created' : '🗓 Post scheduled');
            } catch (e) {
                console.error('Create post error:', e);
                pagesShowToastImpl('Could not publish — check Firestore rules for /posts');
            } finally {
       ...
       function renderTimeCapsulePost() {
        ...
        function pagesStartCapsuleCountdownImpl() {
        ...
        function pagesSwitchTabImpl(name) {
        ...
        async function toggleFollow() {
            if (!pageSlug) {
                ...
                pagesShowToastImpl(following ? '🔔 You are now following this page' : 'Unfollowed');
                return;
            }
            try {
                const authUser = await authReadyPromise;
                if (!authUser) { pagesShowToastImpl('You need to be signed in.'); return; }
                ...
                pagesShowToastImpl(isFollowing ? 'Unfollowed' : '🔔 You are now following this page');
            } catch (e) {
                console.error('toggleFollow error:', e);
                pagesShowToastImpl('Could not update — try again');
            }
        }
        ...
        function setNotifPref(el, label) {
            ...
            pagesShowToastImpl('🔔 Notifications set to: ' + label);
        }
        ...
        function saveLangPref() {
            ...
            pagesShowToastImpl('🌐 Language updated to ' + langNames[selectedLang]);
        }
        ...
        function setTextSize(size) {
            ...
            applyA11yPrefs();
            pagesShowToastImpl('Text size: ' + size);
        }
        ...
        async function submitEditPage() {
            ...
            try {
                ...
                pagesShowToastImpl('Page info updated');
            } catch (e) {
                console.error('submitEditPage error:', e);
                pagesShowToastImpl('Could not save — try again');
            } finally {
        ...
        async function addAdminUsername() {
            ...
            try {
                ...
                input.value = '';
                pagesShowToastImpl(uname + ' is now an admin');
                openRolesModal();
            } catch (e) { pagesShowToastImpl('Could not add admin'); }
        }
        async function removeAdminUsername(uname) {
            try {
                await pageRef().update({ adminUsernames: FieldValue.arrayRemove(uname) });
                pagesShowToastImpl(uname + ' removed as admin');
                openRolesModal();
            } catch (e) { pagesShowToastImpl('Could not remove admin'); }
        }
        ...
        async function cancelScheduledPost(postId) {
            ...
            pagesShowToastImpl('Scheduled post cancelled');
            refreshScheduledCount();
            openScheduledModal();
        }
        ...
        async function submitReport() {
            if (!selectedReportReason) { pagesShowToastImpl('Choose a reason first'); return; }
            try {
                ...
                pagesShowToastImpl('🚩 Report submitted');
            } catch (e) {
                console.error('submitReport error:', e);
                pagesShowToastImpl('Could not submit report — try again');
            }
        }
        ...
        async function confirmBlock() {
            ...
            try {
                ...
                pagesShowToastImpl('⛔ Page blocked');
                checkIfBlocked();
            } catch (e) { pagesShowToastImpl('Could not block — try again'); }
        }
        async function unblockPage() {
            await myBlockedPageRef().delete().catch(() => {});
            pagesShowToastImpl('Page unblocked');
            renderFeedPosts();
        }
        ...
        function toggleSave() {
            ...
            pagesShowToastImpl(saved ? '🔖 Page saved' : 'Removed from saved');
        }
        ...
        // ============================================================
        var toastTimer = null;
        function pagesShowToastImpl(msg) {
        ...
        var groupStoryIndex = 0;
        var groupStoryTimer = null;
        var GROUP_STORY_DURATION = 5000;
        ...
        function initPage() {
            ...
            // MUHIMMI: chats.js (da sauran page scripts) suna da NASU
            // switchTab/showToast/startCapsuleCountdown a GLOBAL scope
            // (babu IIFE, bisa gangan, domin onclick="..." su same su).
            // Duk wanda ya loda a KARSHE (a wannan session) shi ke
            // "mamaye" sunan global din — koda mun koma pages.html bayan
            // haka, tsohon "switchTab" na chats.js na iya zama shi ke
            // aiki. Don haka, a KOWACE shiga pages.html, dole mu SAKE
            // dora wadannan sunaye zuwa NASU (pages.js) version, domin
            // onclick="switchTab(...)"/"showToast(...)" a wannan page
            // su kira daidai function din da ya dace da pages.html.
            window.switchTab = pagesSwitchTabImpl;
            window.showToast = pagesShowToastImpl;
            window.startCapsuleCountdown = pagesStartCapsuleCountdownImpl;
            ...
            initRealPage();
            renderGrids();
            pagesStartCapsuleCountdownImpl();
            applyA11yPrefs();
