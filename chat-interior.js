...
function openMediaLinksDocs() {
    alert('Media, links da docs cikakke gallery view ba a gina shi tukuna a wannan zangon — adadin da kake gani sama daidai ne (real count daga Firestore).');
}
function openStarredMessages() {
    db.collection('personalChats').doc(chatRoomId).collection('messages')
      .where('starredBy.' + myId, '==', true).get().then(snap => {
          if (snap.empty) { alert('Babu sakon da aka yiwa tauraro (star) tukuna a wannan chat.'); return; }
          let list = [];
          snap.forEach(doc => { const d = doc.data(); list.push(d.text || (d.type ? '[' + d.type + ']' : '[sako]')); });
          alert('Sakonnin da aka star:\n\n' + list.join('\n---\n'));
      }).catch(() => alert('An kasa lodawa.'));
}
...
function runChatSearch(query) {
    ...
    resultsEl.innerHTML = matches.length
        ...
        : '<div style="padding:20px;color:rgba(255,255,255,0.3);text-align:center;font-size:12.5px;">Babu sakamako</div>';
}
...
function toggleDisappearingMessages() {
    const hoursStr = prompt('Bayan awanni nawa sakonni za su ɓace ga kai kadai? (0 = kashe)');
    ...
    }).catch(() => alert('An kasa canzawa.'));
}
function toggleChatLock() {
    const key = 'nexus_chatlock_' + chatRoomId;
    const isOn = localStorage.getItem(key) === 'on';
    if (!isOn) {
        const pin = prompt('Kafa PIN don kulle wannan chat a wayarka:');
        if (!pin) return;
        localStorage.setItem(key, 'on');
        localStorage.setItem(key + '_pin', pin);
    } else {
        const pin = prompt('Shigar da PIN don cire kulle:');
        if (pin !== localStorage.getItem(key + '_pin')) { alert('PIN ba daidai ba.'); return; }
...
function toggleTranslateMessages() {
    ...
    if (!isOn) alert('An kunna Translate. Lura: babu translation API da aka haɗa a codebase din nan tukuna, don haka fassarar ba za ta faru ba har sai an haɗa backend translation service.');
}
...
async function exportChatHistory() {
    try {
        ...
        for (const doc of snap.docs) {
            ...
            if (d.encrypted) {
                try { const parsed = await decryptFromPeer(d); text = parsed.text; } catch (e) { text = '🔒 [an kasa buɗewa]'; }
            }
        ...
        alert('An kasa fitar da tattaunawar: ' + e.message);
    }
}
...
function NexusChatInterior_init() {
    ...
    if (localStorage.getItem('nexus_chatlock_' + chatRoomId) === 'on') {
        const pin = prompt('Wannan chat an kulle shi. Shigar da PIN:');
        if (pin !== localStorage.getItem('nexus_chatlock_' + chatRoomId + '_pin')) {
            alert('PIN ba daidai ba.');
