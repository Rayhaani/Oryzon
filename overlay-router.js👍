/* ============================================================
   NEXUS OVERLAY ROUTER — overlay-router.js (v1.2 — PHASE 1, FIX)
   ------------------------------------------------------------
...
(function () {
    ...
    async function buildOverlay(filename, url, key) {
        ...
        wrap.style.display = 'none'; // hidden but IN the document — see comment below
        if (contentEl) wrap.appendChild(contentEl);

        // Attach to the real document BEFORE loading group.js. group.js
        // calls document.getElementById(...) — if its elements aren't in
        // the document yet, those calls return null and any code that
        // uses the result (e.g. someEl.addEventListener(...)) throws,
        // silently killing the REST of group.js's top-level code (which
        // is exactly why real Firestore data + buttons never worked,
        // even though the static HTML/CSS rendered fine on its own).
    ...
    async function open(url) {
        ...
        ensureRoot().appendChild(entry.el); // no-op re-append if already attached (1st build), or actual re-attach (cached reopen)
